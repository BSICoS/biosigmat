...
plot(t, decg);
hold on;
plot(t, upslopes, ':', 'linewidth', 1.5);
plot(t, downslopes, '--', 'linewidth', 1.5);
plot(t(upslopeMaxPosition + nk), upslopes(upslopeMaxPosition + nk), 'v');
plot(t(downslopeMinPosition + nk), downslopes(downslopeMinPosition + nk), '^');
axis tight;
ylabel('1st der ECG');
